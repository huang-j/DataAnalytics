...
covars <- c('gender', 'factor(ecog)', 'factor(factorMet)', 'CA19.300', 'SLD')
covars.col <- paste(covars, collapse="+")
form <- as.formula(paste("Surv(PFS, Progression) ~ exo5+cf+",covars.col,sep=''))
...
covars <- c('factor(ecog)', 'factor(factorMet)', 'CA19.300', 'SLD')
covars.col <- paste(covars, collapse="+")
form <- as.formula(paste("Surv(OS, vital.status) ~ exo5+cf+",covars.col,sep=''))
metos.cox <- coxph(form, data=met )
...
exostage <- ggplot(NULL, aes(y=exop1)) + geom_boxplot(data=met, alpha=.01, aes(x="Unresectable")) + geom_jitter(data=met, size=3, aes(x="Unresectable")) +geom_boxplot(data=resect, alpha=.01, aes(x="Resectable")) + geom_jitter(data=resect, size=3, aes(x="Resectable")) + scale_y_continuous(trans='log10')
